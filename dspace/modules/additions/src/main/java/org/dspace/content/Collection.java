...
public class Collection extends DSpaceObject
{
    ...
    {
        String myQuery = "SELECT i.* FROM collection2item c, item i "
             + "LEFT JOIN metadatavalue v ON (i.item_id=v.item_id "
             + "AND v.metadata_field_id=(SELECT f.metadata_field_id "
             + "FROM metadatafieldregistry f "
             + "WHERE f.element='sortkey' AND f.qualifier is null)) "
             + "WHERE i.item_id=c.item_id AND c.collection_id= ? "
             + "AND i.in_archive='1' ORDER BY cast(v.text_value as integer)";
