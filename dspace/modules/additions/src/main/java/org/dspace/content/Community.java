...
public class Community extends DSpaceObject
{
    ...
    public String getMetadata(String field)
    {
     ...
     /**
     * Get the value of a metadata field with the specified deliminator
     *
     * @param field
     *            the name of the metadata field to get
     * @return the value of the metadata field
     ...
     *                if the requested metadata field doesn't exist
     */
    public String getMetadataForLocale(String field)
    {
        return getMetadataForLocale(field, null);
    }

    /**
     * Get the value of a metadata field for the Locale with deliminator
     *
     * @param field
     *            the name of the metadata field to get
     * @param locale
     ...
     *                if the requested metadata field doesn't exist
     */
    public String getMetadataForLocale(String field, Locale locale)
    {
        String metadata = getMetadata(field);

        if (metadata != null && !metadata.equals(""))
        {
            int pos1 = metadata.indexOf(" = ");
            if (pos1 > -1)
            {
                String lang = null;
                if (locale == null)
                    lang = ourContext.getCurrentLocale().getLanguage();
                else
                    lang = locale.getLanguage();

                if (lang.equals("ja"))
                {
                    metadata = metadata.substring(0, pos1).trim();
                }
                else
                {
                    metadata = metadata.substring(pos1+3).trim();
                }
            }
        }
        else
    ...
    public Collection[] getCollections() throws SQLException
    {
        ...
        TableRowIterator tri = DatabaseManager.queryTable(
            ...
            "ORDER BY collection.short_description DESC, collection.name",
            getID());
    ...
    /**
     * Get the bulletins of this community. Throws an
     * SQLException because creating a community object won't load in all
     * collections.
     * 
     * @return array of Community objects
     */
    public Community[] getBulletins() throws SQLException
    {
        List<Community> bulletins = new ArrayList<Community>();

        Community[] subcommunities = this.getSubcommunities();
        for (Community subcommunity : subcommunities)
        {
            if (subcommunity.isBulletin())
                bulletins.add(subcommunity);
        }

        // Put them in an array
        return bulletins.toArray(new Community[bulletins.size()]);
    }
    
    /**
     * alias to getCollections()
     *
     * @return array of Collection objects
     */
    public Collection[] getIssues() throws SQLException
    {
    ...
    /**
     * return TRUE if this community is a bulletin
     * 
     * @return boolean true = this community is a bulletin
     */
    public boolean isBulletin()
    {
        return communityRow.getBooleanColumn("bulletin");
    }

    /**
     * set boolean field "bulletin"
     * 
     * @param bulletin
     *          true if this community is a bulletin
     */
    public void setBulletin(boolean bulletin)
    {
        communityRow.setColumn("bulletin", bulletin);
        modifiedMetadata = true;
        addDetails("bulletin");
    }

    /**
     * Get the journals of this community. Throws an
     * SQLException because creating a community object won't load in all
     * collections.
     * 
     * @return array of Community objects
     */
    public Community[] getJournals() throws SQLException
    {
        List<Community> journals = new ArrayList<Community>();

        Community[] subcommunities = this.getSubcommunities();
        for (Community subcommunity : subcommunities)
        {
            if (subcommunity.isJournal())
                journals.add(subcommunity);
        }

        // Put them in an array
        return journals.toArray(new Community[journals.size()]);
    }
    
    /**
     * return TRUE if this community is a journal
     * 
     * @return boolean true = this community is a journal
     */
    public boolean isJournal()
    {
        return communityRow.getStringColumn("journal") != null;
    }
     ...
     * @param journal code
     * 
     * @return community or null if 
     */
    public static Community findJournal(Context context, String journal) throws SQLException
    {
        // get all communities that are not children
        TableRow row = DatabaseManager.querySingle(context, 
                "SELECT * FROM community WHERE journal=?", journal.trim());

        if (row == null)
        {
            if (log.isDebugEnabled())
            {
                log.debug(LogManager.getHeader(context, "find_journal",
                        "not_found,journal_code=" + journal));
            }
    ...
    {
        return communityRow.getStringColumn("journal");
    }

    /**
     * return Journal display mode
     * 
     * @return mode
     */
    public String getJournalMode()
    {
        return communityRow.getStringColumn("mode");
    }

    /**
     * set boolean field "journal"
     * 
     * @param journal
     *          true if this community is a journal
     */
    public void setjournal(String journal)
    {
        communityRow.setColumn("journal", journal.trim());
        modifiedMetadata = true;
        addDetails("journal");
    }
